...
namespace boost { namespace simd { namespace detail
{
  ...
  template< typename T1, std::size_t N1, typename X1
          ...
          >
  BOOST_FORCEINLINE pack<T1,N1,X1> bitwise_notand_ ( BOOST_SIMD_SUPPORTS(simd_)
                                                   ...
                                                   ) BOOST_NOEXCEPT
  {
    ...
    return bitwise_notand(a, simd::bitwise_cast<pack<T1,N1,X1>>(b));
  }
  ...
  BOOST_FORCEINLINE
  pack<T1,N,simd_emulation_> bitwise_notand_ ( BOOST_SIMD_SUPPORTS(simd_)
                                             ...
                                             ) BOOST_NOEXCEPT
  {
    ...
    return map_to( simd::bitwise_notand, a, b);
  }
  ...
  >
  BOOST_FORCEINLINE
  pack<T2,M> bitwise_notand_ ( BOOST_SIMD_SUPPORTS(simd_)
                             , T1 a
                             , pack<T2,M> const& b
                             ) BOOST_NOEXCEPT
  {
  ...
  >
  BOOST_FORCEINLINE
  pack<T1,N> bitwise_notand_ ( BOOST_SIMD_SUPPORTS(simd_)
                             , pack<T1,N> const& a
                             , T2 b
                             ) BOOST_NOEXCEPT
  {
