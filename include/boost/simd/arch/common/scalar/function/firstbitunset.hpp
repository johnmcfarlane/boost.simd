...
namespace boost { namespace simd { namespace detail
{
  ...
  template<typename T >
  BOOST_FORCEINLINE
  as_ui_t<T> sfirstbitunset_( T a0
  ...
  {
    using result_t = as_ui_t<T>;
    return sfirstbitunset_(bitwise_cast<result_t>(a0) , std::false_type());
  }

  template<typename T>
  BOOST_FORCEINLINE
  as_ui_t<T> firstbitunset_(BOOST_SIMD_SUPPORTS(cpu_)
         , T a) BOOST_NOEXCEPT
  {
    return sfirstbitunset_(a, std::is_floating_point<T>());
  }
