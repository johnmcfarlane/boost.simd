...
namespace boost { namespace simd { namespace detail
{
 // Native implementation
  ...
  BOOST_FORCEINLINE
  pack<ui_t<T>,N> firstbitunset_(BOOST_SIMD_SUPPORTS(simd_)
  ...
  template<typename T, std::size_t N>
  BOOST_FORCEINLINE
  pack<ui_t<T>,N,simd_emulation_> firstbitunset_ ( BOOST_SIMD_SUPPORTS(simd_)
                                               , pack<T,N,simd_emulation_> const& a
                                               ) BOOST_NOEXCEPT
  {
    return map_to(simd::firstbitunset, a);
  }
