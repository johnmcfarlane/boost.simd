...
namespace boost { namespace simd { namespace detail
{
 ...
 v_is_not_less_ ( pack<T, N, X> const& a
                   , pack<T, N, X> const& b
  ...
  (
    logical_not(is_less(a, b))
  )

  template<typename T, std::size_t N, typename X>
  BOOST_FORCEINLINE  auto
  is_not_less_ ( BOOST_SIMD_SUPPORTS(simd_)
                 , pack<T, N, X> const& a
                 , pack<T, N, X> const& b
                 ) BOOST_NOEXCEPT_DECLTYPE_BODY
  (
    v_is_not_less_(a, b, std::is_floating_point<T>())
  )
  ...
  {
    return is_not_less(a, pack<T,N>(b));
  }
  ...
  {
    return is_not_less(pack<T,N>(a), b);
  }
  ...
  {
    return map_to( simd::is_not_less, a, b);
  }
