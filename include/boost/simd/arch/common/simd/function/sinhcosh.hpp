...
namespace boost { namespace simd { namespace ext
{
  ...
  {
    BOOST_FORCEINLINE detail::sinhcosh_result<A0> operator() ( A0 const& a0) const
    {
      //////////////////////////////////////////////////////////////////////////////
      ...
      if( bs::any(lt1)) s = detail::sinh_kernel<A0>::compute(x, sqr(x));

      auto test1 = x > (Maxlog<A0>()-Log_2<A0>());
      A0 tmp = exp(x*if_else(test1, Half<A0>(), One<A0>()));
      A0 tmp1 = Half<A0>()*tmp;
      A0 rtmp = rec(tmp);
