...
namespace boost { namespace simd { namespace detail
{
  ...
  template<typename T1, typename T2, std::size_t M
           ...
           >
  BOOST_FORCEINLINE
  pack<T2,M> bitwise_xor_ ( BOOST_SIMD_SUPPORTS(simd_)
                          , T1 a
                          , pack<T2,M> const& b
                          ) BOOST_NOEXCEPT
  {
    using p_t =  pack<T2,M>;
    return simd::bitwise_xor(p_t(a), b);
  }
  ...
  template<typename T1, typename T2, std::size_t N
           ...
           >
  BOOST_FORCEINLINE
  pack<T1,N> bitwise_xor_ ( BOOST_SIMD_SUPPORTS(simd_)
                          , pack<T1,N> const& a
                          , T2 b
                          ) BOOST_NOEXCEPT
  {
    using p_t =  pack<T1,N>;
    return simd::bitwise_xor(a, p_t(b));
  }
