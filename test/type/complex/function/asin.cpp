...
STF_CASE_TPL( "Complex asin on complex<T>", STF_IEEE_TYPES)
{
  ...
  STF_ULP_EQUAL( bs::cmplx::asin(c1), bs::complex<T>(0, 1.4436354751788103424932767402731), 0.5);
  STF_ULP_EQUAL( bs::cmplx::asin(c2), bs::complex<T>(- 0.63398386563917671631879711506414, 2.3055090312434769420418359381334), 0.5);
  STF_ULP_EQUAL( bs::cmplx::asin(c3), bs::complex<T>(0.66623943249251525510400489597779, 1.0612750619050356520330189162136), 0.5);
  STF_ULP_EQUAL( bs::cmplx::asin(c4), bs::complex<T>(1.5707963267948966192313216916398, 1.316957896924816708625046347308), 0.5);
}
...
STF_CASE_TPL( "Complex asin complex<pack<T>>", STF_IEEE_TYPES)
{
   ...
   STF_ULP_EQUAL(bs::cmplx::asin(c1), c2, 1.0);
}
