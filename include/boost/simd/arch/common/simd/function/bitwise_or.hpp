...
namespace boost { namespace simd { namespace detail
{
  ...
  template< typename T1, std::size_t N1, typename X1
          ...
          >
  BOOST_FORCEINLINE pack<T1,N1,X1> bitwise_or_ ( BOOST_SIMD_SUPPORTS(simd_)
                                                ...
                                                ) BOOST_NOEXCEPT
  {
    ...
    return bitwise_or(a, simd::bitwise_cast<pack<T1,N1,X1>>(b));
  }
  ...
  BOOST_FORCEINLINE
  pack<T1,N,simd_emulation_> bitwise_or_ ( BOOST_SIMD_SUPPORTS(simd_)
                                          ...
                                          ) BOOST_NOEXCEPT
  {
    ...
    return map_to( simd::bitwise_or, a, b);
  }
  ...
  >
  BOOST_FORCEINLINE
  pack<T2,M> bitwise_or_ ( BOOST_SIMD_SUPPORTS(simd_)
                         , T1 a
                         , pack<T2,M> const& b
                         ) BOOST_NOEXCEPT
  {
    using p_t =  typename std::decay<decltype(b)>::type;
    return simd::bitwise_or(p_t(a), b);
  }
  ...
  >
  BOOST_FORCEINLINE
  pack<T1,N> bitwise_or_ ( BOOST_SIMD_SUPPORTS(simd_)
                         , pack<T1,N> const& a
                         , T2 b
                         ) BOOST_NOEXCEPT
  {
    using p_t =  typename std::decay<decltype(a)>::type;
    return simd::bitwise_or(a, p_t(b));
  }
} } }
