...
#define BOOST_SIMD_FUNCTION_DIV_HPP_INCLUDED
  ...
  /*!
    ...
    With no option this function is equivalent to `divides(x, y)` (See  [divides](@ref real-divides).)

    Options may be  [ceil](@ref real-ceil),  [floor](@ref real-floor),  [fix](@ref real-fix),  [round](@ref real-round),  [nearbyint](@ref real-nearbyint) (in the
