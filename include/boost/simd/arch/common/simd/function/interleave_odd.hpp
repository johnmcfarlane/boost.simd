...
namespace boost { namespace simd { namespace detail
{
  template<typename T> struct v_io
  {
    ...
    {
      return simd::extract<2*(N::value/2)+1>(y);
    }
  ...
  interleave_odd_(BOOST_SIMD_SUPPORTS(simd_)
          , pack<T,N,X> const & x
          , pack<T,N,X> const & y) BOOST_NOEXCEPT
  {
    ...
    return v_io<pack<T,N,X>>::do_( x, y, stk_t{}
