...
STF_CASE_TPL( "pow on complex<T>/complex<T>", STF_IEEE_TYPES)
{
  ...
  cp_t x(2, 3);
  cp_t y(1, 0.25);
  cp_t z(0.74509938112659227173925750456051, 2.7198994461260849240560206508983);
  STF_ULP_EQUAL( bs::pow(x, y), z, 1.0);
...
STF_CASE_TPL( "pow on T/complex<T>", STF_IEEE_TYPES)
{
  ...
  T x(-2);
  cp_t y(1.0, 0.25);
  cp_t z(-0.89821942821603296853913520691995, -0.15722647416485059396946240479643); 
  STF_ULP_EQUAL( bs::pow(x, y), z, 1.0);
...
STF_CASE_TPL( "pow on complex<T>,  T", STF_IEEE_TYPES)
{
  ...
  T y(-0.5);
  cp_t x(1.0, 0.25);
  cp_t z(0.97757837811951609985504014263266, -0.12034539782870095269303954278421);
  STF_ULP_EQUAL( bs::pow(x, y), z, 1.0);
