...
namespace boost { namespace simd { namespace detail
{
  ...
  logical_andnot_ ( BOOST_SIMD_SUPPORTS(simd_)
               , pack<T1,N> const& a0
               , pack<T2,N> const& a1
               ) BOOST_NOEXCEPT
  {
    return mask2logical(bitwise_andnot(genmask(is_nez(a0)) , genmask(is_nez(a1))));
  }
  ...
  {
    return mask2logical(bitwise_andnot(genmask(a0), genmask(is_nez(a1))));
  }
  ...
  {
    return mask2logical(bitwise_andnot(genmask(is_nez(a0)), genmask(a1)));
  }
  ...
  {
    return map_to(logical_andnot, a0, a1);
  }
  ...
  {
    using p_t =  pack<T2,M>;
    return simd::logical_andnot(p_t(a), b);
  }
  ...
  {
    using p_t =  pack<T1,N>;
    return simd::logical_andnot(a, p_t(b));
  }
