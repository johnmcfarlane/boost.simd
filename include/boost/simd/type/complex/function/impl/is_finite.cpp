...
namespace boost { namespace simd { namespace ext
{
                          ...
                          )
  {
    BOOST_FORCEINLINE A0 operator()(A0 const& a0) const BOOST_NOEXCEPT
    {
      ...
      A0 z{if_else(is_real_a0, sqrtx, w), if_else(is_real_a0,Zero<rtype>(), aa0*Half<rtype>()/w));

      z = if_else(gezra0, z, A0(z.imag, z.real));
      if (bs::all(is_finite(z))) return if_else(negimag, conj(z), z);
      z = if_else(iaa0 == Inf<rtype>(),
                  A0(Inf<rtype>(), Inf<rtype>()),
                  z);
      z = if_else(logical_andnot(a0.real ==  Minf<rtype>(), is_nan(iaa0)),
                  A0(if_else_zero(iaa0 ==  Inf<rtype>(), iaa0),
                     Inf<rtype>()
                    ),
                  z);
