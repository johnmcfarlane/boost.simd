...
namespace boost { namespace simd { namespace detail
{
  ...
  BOOST_FORCEINLINE
  pack<U,N> vif_inc_( pack<T,N> const& a0
  ...
  {
    return if_plus(a0, a1, One(as(a1)));
  }

  template<typename T, typename U, std::size_t N>
  BOOST_FORCEINLINE
  pack<U,N> if_inc_(BOOST_SIMD_SUPPORTS(simd_)
                            , pack<T,N> const& a0
                            , pack<U,N> const& a1 ) BOOST_NOEXCEPT
  {
    ...
    return vif_inc_(a0, a1, test_t());
  }

  // Emulated implementation
  template<typename T, typename U, std::size_t N>
  BOOST_FORCEINLINE
  pack<U,N,simd_emulation_> if_inc_ ( BOOST_SIMD_SUPPORTS(simd_)
                                    , pack<T,N,simd_emulation_> const& a0
                                    , pack<U,N,simd_emulation_> const& a1
                                    ) BOOST_NOEXCEPT
  {
    return map_to(simd::if_inc, a0, a1);
  }

  //mixed
  template<typename T, std::size_t N, typename U>
  BOOST_FORCEINLINE pack<T,N>
  if_inc_(BOOST_SIMD_SUPPORTS(simd_)
         , pack<T,N> const & a0
         , U  a1) BOOST_NOEXCEPT
  {
    using p_t = pack<U, N>;
    return if_inc_(a0, p_t(a1));
  }
